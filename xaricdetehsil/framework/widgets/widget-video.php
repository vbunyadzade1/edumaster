...
##Video ------------------------------------------ #
add_action( 'widgets_init', 'kavkaz_video_widget_box' );
function kavkaz_video_widget_box() {
	register_widget( 'kavkaz_video_widget' );
}
class kavkaz_video_widget extends WP_Widget {

	public function __construct(){
		$widget_ops 	= array( 'classname' => 'video-widget', 'description' => ''  );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'video-widget' );
		parent::__construct( 'video-widget', THEME_NAME .' - '.__( 'Video' , 'kavkaz') , $widget_ops, $control_ops );
	}
	...
	public function form( $instance ) {
		$defaults = array( 'title' =>__('Featured Video', 'kavkaz') );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'embed_code' ); ?>"><?php _e( 'Embed Code' , 'kavkaz') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'video_url' ); ?>"><?php _e( 'Video URL' , 'kavkaz') ?></label>
