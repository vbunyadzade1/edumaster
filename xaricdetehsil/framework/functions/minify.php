...
# HTML Minify
/*-----------------------------------------------------------------------------------*/
function kavkaz_remove_script_version( $src )
{
    $parts = explode( '?ver', $src );
    return $parts[0];
}
add_filter( 'script_loader_src', 'kavkaz_remove_script_version', 15, 1 );
add_filter( 'style_loader_src', 'kavkaz_remove_script_version', 15, 1 );
    
function kavkaz_init_minify_html() 
{
    $minify_html_active = get_option('minify_html_active');
    if ( $minify_html_active != 'no') ob_start('kavkaz_minify_html_output');
}

if ( !is_admin() ) add_action('init', 'kavkaz_init_minify_html', 1 );

function kavkaz_minify_html_output($buffer)
{
