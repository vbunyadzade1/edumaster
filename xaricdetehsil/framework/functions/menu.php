...
if (!function_exists('bootstrap_setup')):
    function bootstrap_setup() {
        class kavkaz_menu extends Walker_Nav_Menu {
            ...
            function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
                ...
                $li_attributes = ' itemscope itemtype="https://schema.org/SiteNavigationElement"';
                ...
                $class_names = ($args->has_children) ? ' class="has-children nav-item"' : ' class="nav-item"';
                ...
                $attributes = ($item->current || $item->current_item_ancestor) ? ' class="nav-link active"' : ' class="nav-link"';
                ...
                $dropdown = ($args->has_children) ? '<i class="bx bx-chevron-down"></i>' : '';

                $item_output.= '<a' . $attributes . '><span itemprop="name">';
                $item_output.= $args->link_before . apply_filters('the_title', $item->title, $item->ID) . $args->link_after;
                $item_output.= $dropdown.'</span></a>';
